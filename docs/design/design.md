...
3. **Volume** — abre o popover vertical (seção 6). Abre também **ao passar o mouse**, depois de uma pausa curta de intenção; botão e popover contam como uma região só, então o ponteiro parado não faz o controle piscar, a travessia entre os dois tem uma breve tolerância e sair de ambos fecha — nunca no meio de um arrasto. Aberto por clique, permanece até clique externo ou `Esc`. Ícone fica vermelho quando mudo. O botão **não** ganha destaque enquanto o popover está aberto: o popover vive na janela de overlay, e a parede não guarda esse estado.
