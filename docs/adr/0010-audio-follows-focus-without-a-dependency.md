...
Making audio follow focus — only the game in the OS foreground is audible, the rest muted —
...
- The mute policy lives in the **core** (`Orchestrator.updateAudioFocus`): the foreground pid
  ...
  muted through a new `AudioController` port. One rule, no special case — a non-slot foreground
