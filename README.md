...
Download `Hecaton-<version>-win-x64.zip` from the
[releases page](https://github.com/Shofnip/hecaton/releases/latest), extract it wherever you like,
and double-click `Hecaton.exe` inside the extracted folder. There is nothing to install and nothing
to configure: no setup wizard, no administrator prompt, no Start-menu entry. Put the folder
somewhere with room — it is **809 MB extracted**, and most of that is the browser the app ships.

Updating is replacing the folder with the new one. Removing the app is deleting the folder.

Three things to expect, all of them consequences of a decision rather than accidents:

- **Windows will warn you the first time you run it** — "Windows protected your PC", _More info_ →
  _Run anyway_. The build is not signed, because a code-signing certificate is issued to a verified
  legal identity that every user then sees. For a tool handed to a few friends, that trade was not
  worth making.
- **A `.sha256` is published beside the zip**, and it is the only thing that tells an authentic
  build from a lookalike — anyone can compile this source and produce something that looks the
  same. Check it **before** running anything, since after "Run anyway" it is too late:
  `sha256sum -c Hecaton-<version>-win-x64.zip.sha256`, or `Get-FileHash <file> -Algorithm SHA256` in
...
- **Deleting the folder does not touch your logins.** They live in `%APPDATA%/hecaton`, not beside
  the exe ([ADR-0004](docs/adr/0004-appdata-over-repo-dir.md)), so extracting a newer version keeps
...
clean up. The machine seal in `C:\ProgramData` also stays: it belongs to the machine rather than to
you, holds nothing of yours, and removing it needs an administrator.
